...
const IOS_STANDALONE_TRACKED_KEY = "pwa_ios_standalone_tracked"
...
export default function InstallButton() {
  ...
  useEffect(() => {
    ...
    // Already running as an installed app. Safari never fires an install
    // event, so this standalone-launch check is the only signal we get for
    // iOS installs — log it once per device as a best-effort proxy. This
    // undercounts (only catches people who install *and* reopen), but it's
    // the only thing available on iOS.
    const isStandalone = window.matchMedia("(display-mode: standalone)").matches
    if (isStandalone) {
      if (ios && !localStorage.getItem(IOS_STANDALONE_TRACKED_KEY)) {
        track("pwa_ios_standalone_first_open")
        localStorage.setItem(IOS_STANDALONE_TRACKED_KEY, "1")
      }
      return
    }
  ...
  const handleInstall = async () => {
    ...
    track("pwa_prompt_outcome", { outcome })
    if (outcome === "accepted") {
      localStorage.setItem(INSTALLED_KEY, "1")
      dismiss()
    }
