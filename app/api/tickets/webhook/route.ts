  ...
  try {
    switch (event.type) {
      ...
      case 'checkout.session.completed': {
        ...
        const tierId = meta.tier_id
        const eventId = meta.event_id
        const buyerUserId = meta.buyer_user_id || null // optional now
        const quantity = parseInt(meta.quantity || '1', 10)
        const paymentIntentId =
          typeof session.payment_intent === 'string'
            ? session.payment_intent
        ...
        // Resolve buyer info — see header comment for resolution order.
        const buyerInfo = await resolveBuyerInfo({
          metadata: meta,
          buyerUserId,
          session,
        ...
        const tickets = Array.from({ length: quantity }, () => ({
          ticket_tier_id: tierId,
          event_id: eventId,
          buyer_user_id: buyerUserId, // nullable now
          buyer_email: buyerInfo.email,
          buyer_name: buyerInfo.name,
          buyer_phone: buyerInfo.phone,
          stripe_payment_intent_id: paymentIntentId || null,
          ...
          platform_fee: platformFee ? platformFee / quantity : null,
          status: 'valid' as const,
          // qr_token has a DB default of gen_random_uuid()::text, so we
          // don't need to set it here — Postgres will fill it in.
        }))

        const { error: insertError } = await admin.from('tickets').insert(tickets)
        ...
        console.log(
          `[webhook] minted ${quantity} ticket(s) for event ${eventId} (guest=${!buyerUserId})`
        )
        break
      }
...
async function resolveBuyerInfo(args: {
  ...
  // Try profile fallback if we have a user id
  if (buyerUserId && (!email || !name)) {
    const { data: profile } = await admin
      .from('profiles')
      .select('email, full_name, phone_number')
  ...
  // Last resort: customer_details on the session
  if (session?.customer_details) {
    email = email || session.customer_details.email || null
    name = name || session.customer_details.name || null
    phone = phone || session.customer_details.phone || null
